...
import { useWebformPath,splitDatasourceStr, useIteratorDatasourceGetter, TIteratorDatasourceType } from '@ws-ui/webform-editor';
...
type UseSourcesOptions = Partial<{
  ...
  function useIteratorDatasource({
    type,
    prop = 'datasource',
  }: IUseIteratorDatasourceProps) {
  ...
  export function useSources(options?: UseSourcesOptions) {
    ...
    const ds = iteratorDs
      ? iteratorDs
      : datasource[0]
        ? window.DataSource.getSource(
            datasource[0].id,
            datasource[0].namespace || path,
          )
        : undefined;
    ...
    const currentDs = iteratorCurrentDs
      ...
      : currentElement[0]
        ? window.DataSource.getSource(
            currentElement[0].id,
            currentElement[0].namespace || path,
          )
        : undefined;
